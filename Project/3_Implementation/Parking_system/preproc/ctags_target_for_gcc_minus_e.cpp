# 1 "E:\\Parking_system\\Parking_system.ino"
# 2 "E:\\Parking_system\\Parking_system.ino" 2
# 3 "E:\\Parking_system\\Parking_system.ino" 2
# 4 "E:\\Parking_system\\Parking_system.ino" 2
...
const int LED=3; // Intialize Pin for Led 
 ...
 pinMode(IR1, 0x0); // Configure IR SENSOR 1 to read input 
 pinMode(IR2, 0x0); // Configure IR SENSOR 2 to read input
 ...
 delay (2000);
 lcd.clear();
 pinMode(buzzer, 0x1); // set buzzer -pin as output
 pinMode(LED,0x1);// Set LED - pin  as an output 
 }
 void loop(){

 if(digitalRead (IR1) == 0x0 && flag1==0) // IR SENSOR 1 detects the vehicle it checks slot availability
 {
 if(Slot>0){
  flag1=1;
 if(flag2==0){
 ...
 if(digitalRead (IR2) == 0x0 && flag2==0){flag2=1; // IR sensor 2 will initialize and detects vehicle to enter slot
